...
//	}
	...
	public BillItems postBillItems(BillItems bi) {
		List<DrugModule> li=dmRepository.findAll().stream().filter(e->e.getDrug_id()==bi.getDrug_id()).collect(Collectors.toList());
		DrugModule dm=li.get(0);
	...
	// BillModule----------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		public BillModule postBillmodule(BillModule bm) 
		{
			...
			if(cmRepository.existsById(bm.getCustomer_id())&&bm.getModeofPayment().isEmpty()==false&&g==bm.getBill_amount()) 
			{
				  List<BillItems> li= biRepository.findAll().stream().filter(e->e.getBill_no()==bm.getBill_no()).collect(Collectors.toList());
				 li.forEach(bi->{
				  List<DrugModule> li1=dmRepository.findAll().stream().filter(e->e.getDrug_id()==bi.getDrug_id()).collect(Collectors.toList());
				  DrugModule dm=li1.get(0);
				  dm.setQuantity(dm.getQuantity()-bi.getQuantity());
				  dmRepository.save(dm);
				 });
			      return bmRepository.save(bm);
			}
				throw new DataNotFoundException("BillModule","Id",bm.getCustomer_id());
		}
		@Override
